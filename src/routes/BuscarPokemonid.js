    ...
    useEffect(() => {
        fetch('https://pokeapi.co/api/v2/pokemon/?limit=400')
            ...
            .then((data) => setResult( data.results.map((item) => {
                    fetch(item.url)
                        .then((response) => response.json()).then((allpokemon) => arr.push(allpokemon));
                        setPoke(arr);
                }),
            ));
    }, []);
    setTimeout(() => {
      ...
      return (
        <main>
           ...
           <h1>Escribe el Nombre de algun Pokemon :
           <input style={{width:"200px", height:"40px"}} value={searchParams.get('filter') || ""}
            ...
            /> <small>Si no es posible que se vean los pokemones refresque la pantalla</small>
           </h1>
            { load ? ( 
                ...
                return name.startsWith(filter.toLowerCase());
                  ...
                  <div className='wrapper'>
                    <Link to={"/buscarPokemon/" + pokemon.id}>
                      <div className='card2' id={pokemon.types[0].type.name}  
